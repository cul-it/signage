...
export const actions = {
  async fetchSchedule ({ commit, state }, payload) {
    ...
    if (isEmpty(state) || Robin.staleCache(state.updated) || Robin.pastChange(state.statusChange) || Robin.nextDay(state.updated)) {
      ...
      const schedule = Robin.buildSchedule(feed, payload.location, state, await Robin.openingTime(this.$axios, payload.location, payload.category), await Robin.closingTime(this.$axios, payload.location, payload.category))
